...
```{r debug settings}
# grms and dosages for testing the DirDom model
grms<-list(A=readRDS(file=here::here("output","kinship_A_IITA_2021May13.rds")),
           D=readRDS(file=here::here("output",
                                     "kinship_domGenotypic_IITA_2021July5.rds")))
dosages<-readRDS(file=here::here("data",
...
modelType="DirDom"
blups %<>%
  slice(1:4) %>% # for testing, just 2 traits
...
```
After deploying, I find the AD model works fine. However, DirDom is causing workers to completely detach from the master R session despite continuing.... need to figure this behavior out.

```{r runCrossVal_test - fix DirDom model}
runCrossVal_test<-function(blups,
                           modelType,
                           selInd,SIwts = NULL,
                           grms,dosages=NULL,
                           nrepeats,nfolds,
...
```{r test runCrossVal_test}
testcv_dirdom<-runCrossVal_test(blups=blups,
                                modelType="DirDom",
                                selInd=TRUE,SIwts=SIwts,
                                grms=grms,dosages=dosages,
                                nrepeats=1,nfolds=3,
                                ncores=7,nBLASthreads=5,
                                gid="GID",seed=42)

testcv_ad<-runCrossVal_test(blups=blups,
                                modelType="AD",
                                selInd=TRUE,SIwts=SIwts,
                                grms=grms,dosages=NULL,
                                nrepeats=1,nfolds=3,
                                ncores=3,nBLASthreads=12,
                                gid="GID",seed=42)
```

It seems to work even when there are intentionally data chunks that fails. 
